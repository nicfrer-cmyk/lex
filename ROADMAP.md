...
**Session E — real "open in Word, linked to the site" via WebDAV (user explicitly asked to build this despite the cost/risk tradeoff being spelled out first):**
- New Supabase Edge Function `webdav` (`supabase/functions/webdav/index.ts`, deployed) — a WebDAV protocol bridge in front of Storage, so desktop Word's own `ms-word:ofe|u|<url>` handler can open/edit/save a .docx directly against the site (the same mechanism SharePoint/OneDrive use, not Microsoft-exclusive — Supabase Storage just doesn't speak WebDAV natively). Implements OPTIONS/PROPFIND/GET/HEAD/PUT/LOCK/UNLOCK; deliberately does not implement DELETE/MKCOL/COPY/MOVE (this bridge edits one existing document in place, not general file management).
- New migration `supabase/migrations/20260721000000_webdav_bridge.sql` (also mirrored as `supabase-schema-phase1-fix17.sql`), applied via `supabase db push` — two new, fully additive tables: `webdav_credentials` (Word can't present a Supabase session/JWT, so this is a separate long-lived credential — client generates a random 256-bit token, only its SHA-256 hash is ever sent/stored, shown once in Settings) and `webdav_locks` (minimal single-writer locking so two people can't silently clobber the same open document).
- Settings → "עריכת Word מקושרת לאתר" generates/shows the credential; each .docx's "⋮" menu got "🔗 פתח ב-Word (מקושר לאתר)" alongside the existing download-based edit loop (kept, not replaced — this is Word/docx-specific, the other loop still covers every file type and every browser).
- **What's actually verified vs. not**: the deployed function's routing, Basic-Auth gate, and error handling were tested directly against the live endpoint (OPTIONS with no auth returns the right DAV/MS-Author-Via headers; GET/PROPFIND/PUT all correctly 401 without valid credentials, including malformed auth headers and wrong tokens — no crashes). **Not verified**: the actual authenticated happy path (PROPFIND/GET/PUT/LOCK against a real document) and — the one thing that can only be confirmed by trying it — whether real desktop Word actually completes the `ms-word:ofe` handshake against this server and edits/saves correctly. This was built and deployed without access to a Windows+Word test rig; a real first attempt by the user is the next step, not a formality, and specific errors from that attempt are exactly what's needed to debug anything that doesn't work first try.
...
**Session D — the Content-Type fix above turned out not to be the whole story; user re-tested and it was still just downloading.** Re-architected the viewer properly instead of patching further:
- The real, complete fix: `previewDoc()` no longer feeds a signed Storage URL straight into an `<iframe>`/`<img src>`. It fetches the actual bytes (`Platform.downloadFileBytes()`, the same call the docx/mammoth path already used) and builds a local `blob:` URL with the correct MIME type set client-side, then points the iframe/img at *that*. This doesn't depend on Storage's stored Content-Type at all anymore (the session-A fix still matters for downloads/share-links, where the recipient's own browser/email client is what reads that metadata) — and it's also immune to a second possible failure mode a signed URL can hit that Content-Type alone doesn't explain: a storage provider's response can set its own framing-restriction headers that silently block being embedded in *anyone's* `<iframe>`, no matter what the parent page's own policy allows.
- Net effect: **documents uploaded before the Content-Type fix now preview correctly too, with no re-upload needed** — the earlier advice to re-upload old files via "עדכן גרסה" only still matters for making a *downloaded copy* or a *shared link* recognized correctly by something outside LexTrack (Word, an email client, etc.), not for viewing inside the app.
- Also fixed the e-filing tab's own "פתח" button, which still force-downloaded through the pre-fix code path — it now goes through the same robust preview.
- Verified with real (not mocked-away) PDF bytes fed through `previewDoc()` in actual Chromium: blob URL construction, correct MIME type, iframe load, and blob-URL cleanup on modal close all confirmed. (One caveat surfaced during this verification: *headless* Chromium's PDF viewer behaves differently from a normal Chrome/Edge window and can still show its own "download" affordance for an iframe'd PDF regardless of app code — reproduced with zero LexTrack code involved, a known headless-browser limitation, not something to chase further here.)
